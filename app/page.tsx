...
import { countAssessmentsByClass } from "@/lib/utils/assessmentHelpers";

export default function UstazDashboard() {
  const { user } = useAuth();
  const { ustazClasses, loading, getUstazClasses, assessments } = useData();
        ...
        {selectedClass ? (
          ...
          <div>
            <button
              ...
              className="mb-lg flex items-center gap-2 text-on-surface-variant hover:text-on-surface transition-colors"
            >
              <span className="material-symbols-outlined">arrow_back</span>
              <span className="font-button text-button">ወደ ክፍሎች ተመለስ</span>
            </button>

            <h2 className="font-h2 text-h2 text-on-surface mb-lg flex items-center gap-2">
              <span className="material-symbols-outlined text-primary">menu_book</span>
              {selectedClass.classes?.name || 'Class'}
            </h2>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-md">
              {/* Attendance Card */}
              <Link
                href={`/take-attendance?classId=${selectedClass.class_id}`}
              ...
              >
                <div className="h-2 bg-gradient-to-r from-primary/5 to-transparent"></div>
                <div className="p-lg">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-16 h-16 rounded-xl bg-primary-container/20 text-primary flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300">
                    ...
                    <div>
                      ...
                      <p className="font-body-md text-body-md text-on-surface-variant">
                        {attendanceStatus[selectedClass.class_id] ? 'የዛሬ አቴንዳስ ያሻሽሉ' : 'አዲስ አቴንዳስ ይመዝግቡ'}
                      </p>
                    </div>
                  </div>
                  {attendanceStatus[selectedClass.class_id] && (
                    <div className="p-3 bg-tertiary-container/20 border border-tertiary/30 rounded-lg">
              ...
              {/* Marks Card */}
              <Link
                href={`/take-marks?classId=${selectedClass.class_id}`}
                className="group bg-surface-container-lowest rounded-xl overflow-hidden border border-surface-container-low hover:border-primary/30 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1"
              >
                <div className="h-2 bg-gradient-to-r from-tertiary/5 to-transparent"></div>
                <div className="p-lg">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-16 h-16 rounded-xl bg-tertiary-container/20 text-tertiary flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300">
                      <span className="material-symbols-outlined text-3xl">grade</span>
                    </div>
                    <div>
                      <h3 className="font-h2 text-h2 text-on-surface mb-1 group-hover:text-tertiary transition-colors duration-200">
                        ነጥቦች ይምልክቱ
                      </h3>
                      <p className="font-body-md text-body-md text-on-surface-variant">
                        ፈተናዎች እና ነጥቦች ያስቀምጡ
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-md">
              {loading ? (
                ...
                ustazClasses.map((classUstaz, index) => (
                  <article 
                    key={classUstaz.id} 
                    onClick={() => setSelectedClass(classUstaz)}
                    className="group bg-surface-container-lowest rounded-xl overflow-hidden border border-surface-container-low hover:border-primary/30 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 cursor-pointer"
                  >
